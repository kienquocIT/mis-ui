...
(function (factory) {
    ...
    DmUploaderFile.prototype.upload = async function () {
        var file = this;
        ...
        if (typeof (customData) === "function") {
            // 2023-12-15 : NTTrieu
            // wait input remarks of file before call upload!
            // using async await in extraData for opts call init(opts)
            let returnCustomData = await customData.call(file.widget.element, file.id, file.data);
            if (returnCustomData.state === true){
                // allow upload with custom data
                customData = returnCustomData.data;
            } else {
                // cancel upload
                return false;
            }
