...
(function ($) {
    ...
    TimelineMe.prototype = {
        ...
        init: function () {
            ...
            this.$el.trigger('timelineMe.done');
        },
        ...
        _fillItem: function (item) {
            if ((item.label || item.labelTemplate) && item.labelElement) {
                // item.label ? item.labelElement.html(
                //     `<div class="txt-title ` + item.labelClsName + 
                //     `" data-bs-toggle="tooltip" data-bs-placement="bottom" title="` + item.label + `">` + item.label + `</div>`
                // ) : item.labelElement.html(loadHTMLFromFile(item.labelTemplate));
