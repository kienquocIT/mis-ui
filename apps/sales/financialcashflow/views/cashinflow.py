...
class CustomerAdvanceListForCashInflowAPI(APIView):
    @mask_view(
        auth_require=True,
        is_api=True,
    )
    def get(self, request, *args, **kwargs):
        data = request.query_params.dict()
        resp = ServerAPI(user=request.user, url=ApiURL.CUSTOMER_ADVANCE_LIST_FOR_CASHINFLOW).get(data)
        return resp.auto_return(key_success='customer_advance_list')
