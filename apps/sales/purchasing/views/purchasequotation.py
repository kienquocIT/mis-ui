...
class PurchaseQuotationSaleListAPI(APIView):
    @mask_view(
        auth_require=True,
        is_api=True,
    )
    def get(self, request, *args, **kwargs):
        data = request.query_params.dict()
        resp = ServerAPI(user=request.user, url=ApiURL.PURCHASE_QUOTATION_SALE_LIST).get(data)
        return resp.auto_return(key_success='purchase_quotation_sale_list')
