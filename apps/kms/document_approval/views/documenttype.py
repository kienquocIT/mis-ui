...
class DocumentTypeConfigListAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        resp = ServerAPI(user=request.user, url=ApiURL.KMS_DOC_TYPE_LIST).get(params)
...
class DocumentTypeConfigCreate(View):
    ...
    @mask_view(
        ...
        menu_active='menu_document_type_config',
        breadcrumb='KMS_DOC_TYPE_CREATE_PAGE',
    )
    def get(self, request, *args, **kwargs):
        return {}, status.HTTP_200_OK
...
class DocumentTypeConfigCreateAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        resp = ServerAPI(user=request.user, url=ApiURL.KMS_DOC_TYPE_LIST).post(request.data)
        if resp.state:
...
class DocumentTypeConfigDetailAPI(APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        resp = ServerAPI(user=request.user, url=ApiURL.KMS_DOC_TYPE_DETAIL.fill_key(pk=pk)).get()
        return resp.auto_return()
    ...
    def put(self, request, pk, *args, **kwargs):
        resp = ServerAPI(user=request.user, url=ApiURL.KMS_DOC_TYPE_DETAIL.fill_key(pk=pk)).put(request.data)
        if resp.state:
    ...
    def delete(self, request, pk, *args, **kwargs):
        resp = ServerAPI(user=request.user, url=ApiURL.KMS_DOC_TYPE_DETAIL.fill_key(pk=pk)).delete()
