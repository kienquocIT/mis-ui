$(document).ready(function () {
    ...
    let html_table_contact_mapped = $('#datatable_contact_mapped_list').html();
    ...
    $('#account-owner-id').on('change', function () {
        ...
        let table = $('#datatable_contact_mapped_list').DataTable();
        table.row(0).remove().draw();
        let list_contact_map = getContactMap();
        if (list_contact_map.includes($(this).val())) {
            list_contact_map = list_contact_map.filter(function (obj) {
        ...
        list_contact_map.map(function (id) {
            ...
            table.row.add(data_addition).draw().node();
        })
    });
    ...
    frm.submit(function (event) {
        ...
        let list_contact_id = getContactMap();
        let list_contact_mapped = [];
        list_contact_id.map(function (contact_id) {
            if (contact_id === frm.dataForm['owner_id']) {
                list_contact_mapped.push({
                    'id': contact_id,
                    'is_primary': true,
                })
            } else {
        ...
        frm.dataForm['contact_list'] = list_contact_mapped;
    ...
    $(document).on('click', '#btn-add-contact', function () {
        let list_contact_map = getContactMap();
        $('.input-select-contact').prop('checked', false);
        $('#datatable_contact_list tbody tr').removeClass('selected');
        list_contact_map.map(function (id) {
            let ele_contact = $(`.input-select-contact[data-id="${id}"]`);
            ele_contact.prop('checked', true);
            ele_contact.closest('tr').addClass('selected');
        })

        let contact_owner = $('#account-owner-id').val()
        $(`.input-select-contact[data-id="${contact_owner}"]`).prop('disabled', true);
    });

    $(document).on('change', '.input-select-contact', function () {
        if ($(this).is(':checked')) {
    ...
    $(document).on('click', '#btn-edit-contact', function () {
        let ele_contact_selected = $('.input-select-contact:checked:not(:disabled)');
        let data_contact_map = [];
        ele_contact_selected.each(function () {
            let ele_closest_tr = $((this).closest('tr'));
            data_contact_map.push({
                'id': $(this).data('id'),
                'fullname': ele_closest_tr.find('.span-fullname').text(),
                'job_title': ele_closest_tr.find('.span-job-title').text(),
                'mobile': ele_closest_tr.find('.span-mobile').text(),
                'email': ele_closest_tr.find('.span-email').text(),
            });
        });

        let ele_owner = $('.input-select-contact:disabled');
        let ele_closest_owner = ele_owner.closest('tr');
        data_contact_map.unshift({
            'id': ele_owner.data('id'),
            'fullname': ele_closest_owner.find('.span-fullname').text(),
            'job_title': ele_closest_owner.find('.span-job-title').text(),
            'mobile': ele_closest_owner.find('.span-mobile').text(),
            'email': ele_closest_owner.find('.span-email').text(),
        });

        // load_contact_mapped(data_contact_map);
        let table = $('#datatable_contact_mapped_list').DataTable();
        table.clear().draw();
        data_contact_map.map(function (data) {
