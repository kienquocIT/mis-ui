...
(function ($) {
    ...
    var Formset = function (el, options) {
        ...
        this.$formset = $(el);
        if (this.opts.emptyForm === '[data-subformset-empty-form]') {
            this.$emptyForm = $(this.opts.emptyForm);
        } else {
        ...
        this.formsetPrefix = $(el).data(this.opts.prefix);
    ...
    Formset.prototype.addForm = function () {
        ...
        try{
            ...
            addNewOrderNumber($newFormFragment, thisBody)
        }catch (e) {}
        ...
        $newForm.find('input[type="text"], input[type="number"], select, textarea').addClass('form-control');
        $newForm.find('input[type="checkbox"]').addClass('magic-checkbox');

        // re-load components
        let $select = $newForm.find('[data-ajax]');
        // $.fn.renderFormSelectTypes($select);
        $.fn.renderFormElements($newForm);
    ...
    Formset.prototype.$managementForm = function (name) {
        return $('[name=' + this.formsetPrefix + '-' + name + ']');
    ...
    Formset.prototype.totalFormCount = function () {
        var _this = this;
        let total = 0;
        $('[data-' + this.opts.prefix + '="' + this.formsetPrefix + '"]').each(function () {
            total += $(this).find(_this.opts.body).find(_this.opts.form).length;
        });
        return total;
    ...
    Formset.prototype.animateForms = function () {
        this.$formset.on('formAdded', this.opts.form, function () {
            ...
            try {
                jQuery.fn.formsetAddActions($form);
            } catch (e) {}
            return false;
            ...
            $form.slideUp(0);
            try {
                jQuery.fn.formsetDeleteActions($form);
            } catch (e) {
            }
            return false;
        });
