...
// load common
    static loadDataCommon(frm) {
        if (frm.attr('data-method') === "POST") {
            ...
            GroupLoadDataHandle.loadGroupList();
            ...
            $.fn.callAjax(frm.attr('data-url'), 'GET').then(
                (resp) => {
                    ...
                    if (data) {
                        ...
                        GroupLoadDataHandle.loadGroupLevelList(data.group.group_level?.id);
                        GroupLoadDataHandle.loadGroupList(data.group.parent_n?.id);
                        GroupLoadDataHandle.loadFirstManagerList(data.group.first_manager?.id);
                        GroupLoadDataHandle.loadSecondManagerList(data.group.second_manager?.id);
                        dataTableEmployee();
...
// load group list filter by level
    static loadGroupListFilter(level) {
        if (level) {
...
// load group level list
    static loadGroupLevelList(valueSelected = null) {
...
// load group list
    static loadGroupList(valueSelected = null) {
        let ele = $('#select-box-group');
        let url = ele.attr('data-url');
        let method = ele.attr('data-method');
        $.fn.callAjax(url, method).then(
...
// load first manager list
    static loadFirstManagerList(valueSelected = null) {
...
// load second manager list
    static loadSecondManagerList(valueSelected = null) {
