    ...
    $.fn.callAjax2({
        ...
        if (data && data.hasOwnProperty('form_config_for_entries')) {
            ...
            if (configForEntries) {
                ...
                const hasShowReferrerName = !!(configForEntries?.['display_referrer_name'] || false);
                const hasOwnerRecord = !!(configForEntries?.['display_creator'] || false);
                ...
                configsOrder.map(idx => {
                    ...
                    if (configOfItem && typeof configOfItem === 'object' && configOfItem.hasOwnProperty('type') && typeof configOfItem['type'] === "string" && configOfItem.hasOwnProperty('config') && typeof configOfItem['config'] === 'object' && configOfItem.hasOwnProperty('inputs_data') && Array.isArray(configOfItem['inputs_data'])) {
                        ...
                        inputs_data.map(
                            inputConfig => {
                                if (inputConfig.display === true){
                                    ...
                                    keyMapName[nameOfInput] = inputConfig.label ?? '';
                                    ...
                                    columnsTmp.push({
                                        ...
                                        'render': function (data, type, row) {
                                            ...
                                            if (bodyData) return bodyData?.[nameOfInput] ?? '';
                                            return '';
                                        }
                                    })
                                }
                ...
                let head$ = [
                    ["#", {'r': 2, 'c': 1}],
                    ...headTmp$,
                    [$.fn.gettext('Created time'), {'r': 2, 'c': 1}],
                    [$.fn.gettext('Referrer Name'), {'r': 2, 'c': 1}],
                    [$.fn.gettext('User created'), {'r': 2, 'c': 1}]
                ];
                            ...
                            btnExport$.on('click', function () {
                                $x.fn.showLoadingPage({
                                    didOpenEnd: () => {
                                            ...
                                            (idx) => {
                                                ...
                                                if (configOfField) {
                                                    ...
                                                    if (inputReadyLength === 1){
                                                        mergesOfField = {
                                                            's': {'r': 0, 'c': currentColNum + 1},
                                                            'e': {'r': 1, 'c': currentColNum + 1},
                                                        }
                                                    } else {
                                                        mergesOfField = {
                                                            's': {'r': 0, 'c': currentColNum + 1},
                                                            'e': {'r': 0, 'c': currentColNum + 1 + inputReadyLength - 1},
                                                        }
                                                    }
                                        ...
                                        merges.push({'s': {'r': 0, 'c': currentCol + 2}, 'e': {'r': 1, 'c': currentCol + 2}});
                                        dataHeading.push($.fn.gettext('User created'));
                                        merges.push({'s': {'r': 0, 'c': currentCol + 3}, 'e': {'r': 1, 'c': currentCol + 3}});
                                        // -- push system cols
                                        ...
                                        tbl$.DataTable().rows().data().map((item, idx) => {
                                            ...
                                            keyNameOrder.map(inputName => {
                                                itemDataReverted.push(bodyData?.[inputName] || '');
                                            })
                                        ...
                                        const workbook = $x.cls.excelJS.createWorkbook({
                                            ...
                                            'merges': merges.map(
                                                item => $x.cls.excelJS.indexToCoordRange(item.s, item.e)
                                            ),
                                            'styles': [
