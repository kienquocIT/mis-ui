...
$(function () {
    $(document).ready(function () {
        ...
        $('#chooseCompanyPage').select2();
        $('#chooseStateUser').select2();
        ...
        function load_user_of_tenant() {
            if (!$.fn.DataTable.isDataTable('#tbl-user-of-tenant')) {
                let dtb_user_tenant = $('#tbl-user-of-tenant');
                let frm_user_tenant = new SetupFormSubmit(dtb_user_tenant);
                dtb_user_tenant.DataTable({
                    searching: true,
                    ordering: false,
                    paginate: true,
                    pageLength: 25,
                    ...
                    language: {
                        ...
                        "sSearch": "",
                        "searchPlaceholder": $('#notify-search-text').text(),
                    },
                    ajax: {
                        url: frm_user_tenant.dataUrl, type: frm_user_tenant.dataMethod, dataSrc: function (resp) {
                    ...
                    columns: [{
                        ...
                        data: 'company_list', render: (data, type, row, meta) => {
        ...
        function load_company_list_overview() {
            if (!$.fn.DataTable.isDataTable('#datable_company_overview_list')) {
                ...
                let total_employee_linked = 0;
                let dtb = $('#datable_company_overview_list');
                let frm = new SetupFormSubmit(dtb);
                dtb.DataTable({
                    searching: false, ordering: false, paginate: false, processing: false, language: {
                    ...
                    ajax: {
                        url: frm.dataUrl, type: 'GET', dataSrc: function (resp) {
                            let data = $.fn.switcherResp(resp);
                            if (data) {
                    ...
                    columns: [
                        {
                            data: 'code', width: '10%', className: 'wrap-text', render: (data, type, row, meta) => {
                                return `<a href="{0}" class="btn btn-flush-primary">{1}</a>`.format_by_idx(frm.getUrlDetail(row.id), data)
                            }
                        },
                        {
                        ...
                        },
                        {
                            data: 'employee_linked_user', width: '10%', render: (data, type, row, meta) => {
                                ...
                                total_employee_linked += row['employee_linked_user'];
                                return String.format(`<span class="badge badge-soft-indigo w-100">{0} / {1}</span>`, row['employee_linked_user'], row['employee'],)
                            }
                        },
                    ],
                    footerCallback: function (row, data, start, end, display) {
                        $(this.api().column(3).footer()).html('<span class="badge badge-success w-50">' + total_user_summary + '</span>');
                        $(this.api().column(4).footer()).html('<span class="badge badge-warning w-50">' + power_user_summary + '</span>');
                        $(this.api().column(5).footer()).html('<span class="badge badge-indigo w-100">' + total_employee_linked + ' / ' + total_employee + '</span>');
                    },
                });
            }
        }
            ...
            if (!$.fn.DataTable.isDataTable('#dttEmployeeOfCompany')) {
                ...
                tbl.DataTable({
                    ...
                    dom: "<'row mt-3 miner-group'<'col-sm-12 col-md-3 col-lg-2 mt-3'f>>" +
                        ...
                        "<'row mt-3'<'col-sm-12'tr>>" + "<'row mt-3'<'col-sm-12 col-md-6'i>>",
                    language: {
                        "sProcessing": $('#notify-loading-text').text(),
                        "sLengthMenu": "Hiển thị _MENU_ bản ghi",
                        "sZeroRecords": "Không tìm thấy dữ liệu",
                        "sInfo": "Hiển thị _START_ đến _END_ của _TOTAL_ bản ghi",
                        "sInfoEmpty": "Hiển thị 0 đến 0 của 0 bản ghi",
                ...
                tbl.on('init.dt', function () {
                    $('#waiter-miner-group').children().addClass('col-sm-12 col-md-3 col-lg-2 mt-3').appendTo('.miner-group');
        ...
        $('#tbl-user-of-tenant').on('click', '.btn-edit-popup', function () {
        ...
        $('#chooseCompanyPage').on('change', function (event) {
            $('#dttEmployeeOfCompany').DataTable().ajax.reload();
        });
        $('#chooseStateUser').on('change', function (event) {
            $('#dttEmployeeOfCompany').DataTable().ajax.reload();
        })
