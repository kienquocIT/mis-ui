class PurchaseRequestLoadPage {
    ...
    static loadSupplier(data) {
        this.supplierSelectEle.initSelect2({
            ...
            ele_parent.find('a').attr('href', PurchaseRequestLoadPage.urlEle.data('url-account-detail').format_url_with_uuid(supplier_current.id));
...
function loadSOProduct(dict_so, dict_so_product, id, table) {
    table.clear().draw();
    if (dict_so.hasOwnProperty(id)) {
        ...
        if (id !== undefined) {
            let url = PurchaseRequestLoadPage.urlEle.data('url-so-product').format_url_with_uuid(id);
            $.fn.callAjax2({
                'url': url,
                'method': 'GET',
            }).then((resp) => {
...
function updateURLParameter(paramValue) {
    let param = $.param({
        'type': paramValue
    })
    let url = "{0}?{1}".format_by_idx(PurchaseRequestLoadPage.urlEle.data('url-this-page'), param);
...
function getDataForm(dataForm, ele_request_for, ele_sale_order) {
...
function loadProductDetail(ele_tr_current, ele_url, product) {
...
function loadDtbPrProductDetail(product_datas) {
    if (!$.fn.DataTable.isDataTable('#datatable-pr-product')) {
        let $table = $('#datatable-pr-product')
        $table.DataTableDefault({
            rowIdx: true,
