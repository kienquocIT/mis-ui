    ...
    static LoadOutsourcingMaterialTable(data_list=[], option='create') {
        ...
        material_table_outsourcing.DataTableDefault({
            ...
            columns: [
                ...
                {
                    'render': (data, type, row) => {
                        return `<span class="badge badge-light material-code w-100">${row?.['material']?.['code'] ? row?.['material']?.['code'] : ''}</span>`;
                    }
                },
                {
                    'render': () => {
                ...
                {
                    'render': (data, type, row) => {
                        return `<textarea ${option === 'detail' ? 'disabled readonly' : ''} class="form-control material-note small">${row?.['note'] ? row?.['note'] : ''}</textarea>`;
                    }
                },
                {
                    className: 'text-right',
        ...
        select_material_table.DataTableDefault({
            ...
            initComplete: function () {
                select_material_table.find('tbody tr').each(function () {
                    BOMLoadTab.LoadUOM($(this).find('.material-uom'), $(this).find('.material-uom').attr('data-group-id'))
                })
                for (let i = 0; i < selected_list.length; i++) {
                    select_material_table.find('tbody tr').each(function () {
                        if ($(this).find('.material-checkbox').attr('data-material-id') === selected_list[i]?.['material_id']) {
                            ...
                            BOMLoadTab.LoadUOM($(this).find('.material-uom'), selected_list[i]?.['uom_data']?.['group_id'], selected_list[i]?.['uom_data'])
            ...
            replacement_material_table.DataTableDefault({
                ...
                columns: [
                    ...
                    },
                    {
                        className: '',
                        'render': (data, type, row) => {
                            return `<textarea disabled class="form-control replacement-note small"></textarea>`;
                        }
                    },
                ],
                initComplete: function () {
                    ...
                    for (let i = 0; i < selected_list.length; i++) {
                        replacement_material_table.find('tbody tr').each(function () {
                            if ($(this).find('.replacement-checkbox').attr('data-material-id') === selected_list[i]?.['material_id']) {
                                ...
                                BOMLoadTab.LoadUOM($(this).find('.replacement-uom'), selected_list[i]?.['uom_data']?.['group_id'], selected_list[i]?.['uom_data'])
        ...
        select_tool_table.DataTableDefault({
            ...
            initComplete: function () {
                select_tool_table.find('tbody tr').each(function () {
                    BOMLoadTab.LoadUOM($(this).find('.tool-uom'), $(this).find('.tool-uom').attr('data-group-id'))
                })
                for (let i = 0; i < selected_list.length; i++) {
                    select_tool_table.find('tbody tr').each(function () {
                        if ($(this).find('.tool-checkbox').attr('data-tool-id') === selected_list[i]?.['tool_id']) {
                            ...
                            BOMLoadTab.LoadUOM($(this).find('.tool-uom'), selected_list[i]?.['uom_data']?.['group_id'], selected_list[i]?.['uom_data'])
...
$(document).on("click", '.add-new-material', function () {
    ...
    BOMLoadTab.LoadMaterialSelectTable(selected_list)
})
...
$(document).on("click", '.add-new-swap-material', function () {
    ...
    BOMLoadTab.LoadMaterialReplacementTable([], root_material_id, selected_list)
})
...
$(document).on("click", '.add-new-tool', function () {
    ...
    BOMLoadTab.LoadToolSelectTable(selected_list)
})
