...
class APInvoiceListAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        resp = ServerAPI(user=request.user, url=ApiURL.AP_INVOICE_LIST).get(params)
    ...
    def post(self, request, *arg, **kwargs):
        resp = ServerAPI(user=request.user, url=ApiURL.AP_INVOICE_LIST).post(request.data)
        if resp.state:
            resp.result['message'] = SaleMsg.AP_INVOICE_CREATE
            return resp.result, status.HTTP_201_CREATED
        return resp.auto_return()


class APInvoiceCreate(View):
...
class APInvoiceDetailAPI(APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        resp = ServerAPI(user=request.user, url=ApiURL.AP_INVOICE_DETAIL.fill_key(pk=pk)).get()
        return resp.auto_return(key_success='ap_invoice_detail')
    ...
    def put(self, request, pk, *arg, **kwargs):
        data = request.data
        resp = ServerAPI(user=request.user, url=ApiURL.AP_INVOICE_DETAIL.fill_key(pk=pk)).put(data)
        if resp.state:
...
class GoodReceiptsListForAPInvoiceAPI(APIView):
    ...
    def get(self, request, *args, **kwargs):
        params = request.query_params.dict()
        resp = ServerAPI(user=request.user, url=ApiURL.GOOD_RECEIPT_LIST_AP_INVOICE).get(params)
