                ...
                },
                {
                    ...
                    render: (data, type, row, meta) => {
                        return `<a 
                                   ...
                                   data-checkin-gr-start="${row?.['checkin_gr_start'] ? row?.['checkin_gr_start'].slice(0, 5) : ''}"
                                   data-checkin-gr-end="${row?.['checkin_gr_end'] ? row?.['checkin_gr_end'].slice(0, 5) : ''}" 
                                   data-checkin-threshold="${row?.['checkin_threshold']}" 
                                   data-breakin-time="${row?.['break_in_time'].slice(0, 5)}" 
                                   data-breakin-gr-start="${row?.['break_in_gr_start'] ? row?.['break_in_gr_start'].slice(0, 5) : ''}"
                                   data-breakin-gr-end="${row?.['break_in_gr_end'] ? row?.['break_in_gr_end'].slice(0, 5) : ''}" 
                                   data-breakin-threshold="${row?.['break_in_threshold']}"
                                   data-breakout-time="${row?.['break_out_time'].slice(0, 5)}" 
                                   data-breakout-gr-start="${row?.['break_out_gr_start'] ? row?.['break_out_gr_start'].slice(0, 5): ''}"
                                   data-breakout-gr-end="${row?.['break_out_gr_end'] ? row?.['break_out_gr_end'].slice(0, 5) : ''}" 
                                   data-breakout-threshold="${row?.['break_out_threshold']}"
                                   data-checkout-time="${row?.['checkout_time'].slice(0, 5)}" 
                                   data-checkout-gr-start="${row?.['checkout_gr_start'] ? row?.['checkout_gr_start'].slice(0, 5) : ''}"
                                   data-checkout-gr-end="${row?.['checkout_gr_end'] ? row?.['checkout_gr_end'].slice(0, 5) : ''}" 
